...
int main() {
	...
	float a_float, b_float, c_float, d_float, e_float;
	...
	for (i = 0; i < NUMBER_OF_TRIALS; i++) {
		...
		REPEAT_10(Data = buffer[Addr]);	//To evaluate Timing Overhead

		//c_int = a_int + b_int; // Adds Integers
		//e_float = a_float/c_float + b_float/d_float; // Adds Floats
		//Data = buffer[Addr]; // Reads Random Memory Address
		//XGpio_DiscreteWrite(&Gpio, LED_CHANNEL, 0x1); //Turns on one LED
		/*
		//USB Writes
		for (j = 0; j < 10; j++){
			USB_string = (char)(rand()%95 + 32);
			xil_printf("%c", USB_string);
		}*/
