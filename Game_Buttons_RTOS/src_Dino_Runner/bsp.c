...
// In your timer interrupt handler, add GAME_TICK events:
void TimerCounterHandler(void *data) {
        tickCounter++;
        // Post GAME_TICK every ~50ms for smooth gameplay
        if (tickCounter % 100 == 0) {  // Adjust timing as needed
            QActive_postISR((QActive *)&AO_Lab2A, GAME_TICK_SIG);
        }
}
